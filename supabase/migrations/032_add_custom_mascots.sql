...
CREATE INDEX IF NOT EXISTS idx_mascots_owner_id ON public.mascots(owner_id);
...
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON public.mascots;
...
DROP POLICY IF EXISTS "Enable update for owners" ON public.mascots;
...
DROP POLICY IF EXISTS "Enable delete for owners" ON public.mascots;
