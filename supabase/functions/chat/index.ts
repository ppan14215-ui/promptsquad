...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Auth: validate user JWT when present (x-user-token or _authAccessToken). Do not use anon
    // Bearer for getUser() — that never returns a user.
    let userId = '';
    ...
    // Fallback: manual decode (base64url-safe) when getUser did not resolve
    if (!userId && token) {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const TAG_SAFE_TAIL = 20; // chars kept buffered to avoid splitting an in-progress tag across chunks
        ...
        try {
          ...
          // Flush any remaining buffered text; closes out reasoning or content cleanly.
